...
import { ArrowLeft, Check, ShieldCheck, Loader2, Calendar as CalendarIcon, Clock } from "lucide-react";
import { z } from "zod";
...
const formSchema = z.object({
  first_name: z.string().trim().min(1, "First name is required").max(100).regex(nameRegex, "Letters only"),
  last_name: z.string().trim().min(1, "Surname is required").max(100).regex(nameRegex, "Letters only"),
  ...
  notFoundComponent: () => (
    <div className="min-h-screen grid place-items-center bg-background text-foreground">
      <div className="text-center">
        ...
        <Link to="/" className="mt-4 inline-block text-primary underline">Back to base</Link>
      </div>
    </div>
  ),
  errorComponent: ({ error }) => (
    <div className="min-h-screen grid place-items-center bg-background text-foreground p-6">
      <div className="max-w-md text-center">
        ...
        <Link to="/" className="text-primary underline">Back to base</Link>
      </div>
    </div>
  ),
});
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-background/90 backdrop-blur-md">
        <div className="max-w-3xl mx-auto px-4 md:px-8 h-14 flex items-center justify-between">
          ...
          <div className="text-center">
            ...
            <div className="text-[9px] font-mono uppercase tracking-[0.25em] text-muted-foreground">{meta.tagline}</div>
          </div>
          <div className="w-12" />
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 md:px-8 py-10">
        {submitted ? (
          <div className="bg-card border-2 border-primary p-8 text-center shadow-[var(--shadow-hero)]">
            ...
            <Link to="/" className="mt-6 inline-block text-xs font-bold uppercase tracking-widest text-accent underline">
              Back to home
            </Link>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={onSubmit} className="bg-card border-2 border-border p-6 md:p-8 space-y-6 shadow-[var(--shadow-pop)]">
              <SectionHeader>Who you are</SectionHeader>
              <div className="grid sm:grid-cols-2 gap-4">
                <Field label="First name">
                  <input required maxLength={100} value={firstName} onChange={(e) => setFirstName(e.target.value)} pattern="[A-Za-zÀ-ÿ' \-]+" title="Letters only" className="qinput" />
                </Field>
                <Field label="Surname">
                  <input required maxLength={100} value={lastName} onChange={(e) => setLastName(e.target.value)} pattern="[A-Za-zÀ-ÿ' \-]+" title="Letters only" className="qinput" />
                </Field>
              </div>

              <div className="grid sm:grid-cols-2 gap-4">
                <Field label="Email address">
                  <input required type="email" maxLength={255} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="qinput" />
                </Field>
              ...
              <div className="pt-2">
                ...
                <div className="grid sm:grid-cols-[1fr_120px_140px] gap-4">
                  <Field label="Street name">
                    <input required maxLength={200} value={fromStreet} onChange={(e) => setFromStreet(e.target.value)} placeholder="Baker Street" className="qinput" />
                  </Field>
                  <Field label="Number">
                    <input required maxLength={20} value={fromNumber} onChange={(e) => setFromNumber(e.target.value)} placeholder="221B" className="qinput" />
                  </Field>
                  <Field label="Postcode">
                    <input required maxLength={12} value={fromPostcode} onChange={(e) => setFromPostcode(e.target.value.toUpperCase())} pattern="[A-Za-z0-9 \-]{3,12}" placeholder="NW1 6XE" className="qinput" />
                  </Field>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-[1fr_120px_140px] gap-4">
                  <Field label="Street name">
                    <input required maxLength={200} value={toStreet} onChange={(e) => setToStreet(e.target.value)} placeholder="Abbey Road" className="qinput" />
                  </Field>
                  <Field label="Number">
                    <input required maxLength={20} value={toNumber} onChange={(e) => setToNumber(e.target.value)} placeholder="3" className="qinput" />
                  </Field>
                  <Field label="Postcode">
                    <input required maxLength={12} value={toPostcode} onChange={(e) => setToPostcode(e.target.value.toUpperCase())} pattern="[A-Za-z0-9 \-]{3,12}" placeholder="NW8 9AY" className="qinput" />
                  </Field>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <AddonCheckbox checked={packaging} onChange={setPackaging} title="I need packaging for everything" description="Boxes, bubble wrap, tape and the crew packing it for you." />
                  <AddonCheckbox checked={cleaning} onChange={setCleaning} title="End-of-tenancy cleaning" description="Deep clean after the move so you get your deposit back." />
                  <AddonCheckbox checked={handyman} onChange={setHandyman} title="Handyman services" description="Furniture assembly, mounting, small repairs at the new place." />
                </div>
              </div>

              <Field label="Anything else? (optional)">
                <textarea rows={4} maxLength={2000} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Access, parking, extra stops, fragile items…" className="qinput resize-none" />
              </Field>
...
function SectionHeader({ children, accent }: { children: React.ReactNode; accent?: "primary" | "accent" }) {
  const color = accent === "primary" ? "text-primary" : accent === "accent" ? "text-accent" : "text-foreground";
  return (
    <div className={`mb-3 font-display text-lg uppercase tracking-wide ${color} flex items-center gap-2`}>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="block text-[10px] font-mono uppercase tracking-[0.25em] text-accent mb-1.5">{label}</span>
      {children}
    </label>
  );
}

function AddonCheckbox({ checked, onChange, title, description }: { checked: boolean; onChange: (v: boolean) => void; title: string; description: string }) {
  return (
    <label className={`flex items-start gap-3 p-4 border-2 cursor-pointer transition-all ${checked ? "border-primary bg-primary/10" : "border-border bg-background/40 hover:border-accent/60"}`}>
      <input type="checkbox" checked={checked} onChange={(e) => onChange(e.target.checked)} className="mt-0.5 size-4 accent-primary cursor-pointer" />
      <span>
